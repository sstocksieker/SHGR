...
Pval_RDC = rep(0,p)
for (i in (1:p)){
  corr = AlterCorr(data[,i], data[,-i], type = "RDC")
  Corr_RDC[i] = corr$Correlation
  Pval_RDC[i] = corr$pvalue
}
# ## MIC
...
write.csv(Pval_RDC, file = paste0(rep_fromR,"Pval_RDC_M.csv"), row.names = F)
# ## MIC
