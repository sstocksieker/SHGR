...
# rep_courant <- 'C:/Users/samgo/OneDrive/Perso/Thèse/Travaux/GHGR/Code/sansBruit/pyToR' #dirname(script_path)
...
X = data[,-p]
y=data[,p]
ref_Xy = AlterCorr(y, X, type = "RDC")$Correlation
Corr_RDC = rep(0,p-1)
for (i in (1:(p-1))){
    Corr_RDC[i] = ref_Xy - AlterCorr(y, X[,-i], type = "RDC")$Correlation
}
# ## MIC
...
## RDC
write.csv(Corr_RDC, file = paste0(rep_fromR,"Corr_RDC_M.csv"), row.names = F)
# ## MIC
