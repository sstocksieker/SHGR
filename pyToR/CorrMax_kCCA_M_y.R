...
X = data[,-p]
y=data[,p]
ref_Xy = kcca(as.matrix(X),y,ncomps=2)@kcor[2]
Corr_kCCA = rep(0,p-1)
for (i in (1:(p-1))){
  Corr_kCCA[i] = ref_Xy - kcca(as.matrix(X[,-i]),y,ncomps=2)@kcor[2]
}
...
## kCCA
write.csv(Corr_kCCA, file = paste0(rep_fromR,"Corr_kCCA_M.csv"), row.names = F)
# ## kMMD
