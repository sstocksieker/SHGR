...
X = data[,-p]
y=data[,p]
ref_Xy = AlterCorr(y, X, type = "MIC")$Correlation
Corr_MIC = rep(0,p-1)
for (i in (1:(p-1))){
  Corr_MIC[i] = ref_Xy - AlterCorr(y, X[,-i], type = "MIC")$Correlation
}
# ## dCor
...
## MIC
write.csv(Corr_MIC, file = paste0(rep_fromR,"Corr_MIC_M.csv"), row.names = F)
# ## dCor
