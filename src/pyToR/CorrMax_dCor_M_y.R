...
X = data[,-p]
y=data[,p]
ref_Xy = AlterCorr(y, X, type = "dCor")$Correlation
Corr_dCor = rep(0,p-1)
for (i in (1:(p-1))){
  Corr_dCor[i] = ref_Xy - AlterCorr(y, X[,-i], type = "dCor")$Correlation
}
# ## ACE
...
## dCor
write.csv(Corr_dCor, file = paste0(rep_fromR,"Corr_dCor_M.csv"), row.names = F)
# ## ACE
